...
export default function FirstAgentPage() {
  return (
    <div className="flex flex-col gap-8 max-w-3xl">
      ...
      <div>
        <h1 className="text-4xl font-black text-text-primary tracking-tighter mb-4">Your First Agent</h1>
      ...
      <section>
        <h2 className="text-xl font-bold text-text-primary mb-4">How x402 Works</h2>
        <pre className="bg-dark-50 p-6 rounded-xl border border-surface-border font-mono text-sm text-text-primary overflow-x-auto"><code>{`Agent → GET /api/weather
...
Agent ← HTTP 200 OK { data: "sunny, 25°C" }`}</code></pre>
      ...
      <section>
        <h2 className="text-xl font-bold text-text-primary mb-4">Step 1: Set Up the Contract</h2>
        <p className="text-text-secondary mb-3">
          Follow the <Link href="/docs/installation" className="text-accent-fg hover:underline">Installation</Link> guide to deploy your contract. Make sure you have:
        </p>
      ...
      <section>
        <h2 className="text-xl font-bold text-text-primary mb-4">Step 2: Configure Spending Policies</h2>
        <pre className="bg-dark-50 p-6 rounded-xl border border-surface-border font-mono text-sm text-text-primary overflow-x-auto"><code>{`# Set daily limit to 5 USDC (50,000,000 stroops)
...
stellar contract invoke --id <CONTRACT> --source <OWNER_KEY> \\
  --network testnet -- whitelist_merchant --merchant <MERCHANT_ADDR>`}</code></pre>
      ...
      <section>
        <h2 className="text-xl font-bold text-text-primary mb-4">Step 3: Make Your First Payment</h2>
        <p className="text-text-secondary mb-3">
          The agent calls <code className="bg-dark-200 px-1 rounded text-xs">authorize_payment</code> on the contract. The contract validates all policies and executes the USDC transfer:
        </p>
        <pre className="bg-dark-50 p-6 rounded-xl border border-surface-border font-mono text-sm text-text-primary overflow-x-auto"><code>{`import { authorizePayment } from "./stellar/contract.js";
...
try {
  ...
  // e.g. "ExceedsMaxTx", "MerchantNotWhitelisted"
      ...
      <section>
        <h2 className="text-xl font-bold text-text-primary mb-4">Step 4: Handle the Full x402 Flow</h2>
        <pre className="bg-dark-50 p-6 rounded-xl border border-surface-border font-mono text-sm text-text-primary overflow-x-auto"><code>{`// 1. Request a protected resource
...
if (response.status === 402) {
  ...
  console.log("Got data:", await data.json());
      ...
      <section>
        <h2 className="text-xl font-bold text-text-primary mb-4">Step 5: Monitor via Dashboard</h2>
        <p className="text-text-secondary mb-3">
          Open <code className="bg-dark-200 px-1 rounded text-xs">http://localhost:3000/dashboard</code> to see:
        </p>
        <ul className="text-text-secondary space-y-1 list-disc list-inside">
        ...
        <div className="bg-success-glow border border-success/20 rounded-xl p-4 mt-4">
          ...
          <ul className="text-sm text-text-secondary space-y-1">
            <li><Link href="/docs/x402-middleware" className="text-accent-fg hover:underline">x402 Middleware</Link> — protect your own Express endpoints with x402 paywalls</li>
            <li><Link href="/docs/mcp-integration" className="text-accent-fg hover:underline">MCP Integration</Link> — connect SpendGuard to Claude, GPT, or any MCP-compatible agent</li>
          </ul>
        </div>
      </section>
    </div>
