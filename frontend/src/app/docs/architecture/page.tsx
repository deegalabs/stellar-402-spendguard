export default function ArchitecturePage() {
  return (
    <div className="flex flex-col gap-8 max-w-3xl">
      ...
      <div>
        <h1 className="text-4xl font-black text-text-primary tracking-tighter mb-4">Architecture</h1>
      ...
      <section>
        <h2 className="text-xl font-bold text-text-primary mb-4">System Diagram</h2>
        <div className="bg-dark-50 p-6 rounded-xl border border-surface-border font-mono text-sm overflow-x-auto">
          <pre className="text-text-primary"><code>{`┌─────────────────────────────────────────────────────────────┐
│                      Frontend (Next.js)                     │
...
└─────────────────────────────────────────────────────────────┘`}</code></pre>
      ...
      <section>
        <h2 className="text-xl font-bold text-text-primary mb-4">Component Boundaries</h2>
        <div className="overflow-x-auto rounded-xl border border-surface-border">
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-surface-border">
              <tr>
                <td className="px-4 py-2 font-bold text-text-primary">Contract</td>
                <td className="px-4 py-2 text-text-secondary">Policy enforcement, USDC transfers, audit events</td>
                <td className="px-4 py-2 text-text-secondary">Nothing — validates everything</td>
              </tr>
              <tr>
                <td className="px-4 py-2 font-bold text-text-primary">Backend</td>
                <td className="px-4 py-2 text-text-secondary">API layer, transaction signing, event streaming</td>
                <td className="px-4 py-2 text-text-secondary">Contract for policy enforcement, Horizon for events</td>
              </tr>
              <tr>
                <td className="px-4 py-2 font-bold text-text-primary">Frontend</td>
                <td className="px-4 py-2 text-text-secondary">Dashboard, configuration UI, demo visualization</td>
                <td className="px-4 py-2 text-text-secondary">Backend API, Freighter for wallet signing</td>
              </tr>
              <tr>
                <td className="px-4 py-2 font-bold text-text-primary">Agent</td>
                <td className="px-4 py-2 text-text-secondary">Detects 402 responses, triggers payments</td>
                <td className="px-4 py-2 text-text-secondary">Contract to enforce limits (agent can&apos;t bypass)</td>
              </tr>
              <tr>
                <td className="px-4 py-2 font-bold text-text-primary">MCP Server</td>
                <td className="px-4 py-2 text-text-secondary">Exposes SpendGuard tools to AI agents</td>
                <td className="px-4 py-2 text-text-secondary">Contract for policy enforcement</td>
              </tr>
            </tbody>
          </table>
        </div>
      ...
      <section>
        <h2 className="text-xl font-bold text-text-primary mb-4">Key Design Decisions</h2>
      ...
      <section>
        <h2 className="text-xl font-bold text-text-primary mb-4">Data Flow: x402 Payment</h2>
        <ol className="text-text-secondary space-y-1 list-decimal list-inside">
          <li>Agent sends HTTP request to merchant endpoint</li>
          <li>Merchant returns <strong className="text-text-primary">HTTP 402</strong> with x402 challenge (price, payTo)</li>
          <li>Agent calls <code className="bg-dark-200 px-1 rounded text-xs">authorize_payment(price, merchant)</code> on SpendGuard</li>
          <li>Contract validates: not paused, whitelisted, within limits, sufficient balance</li>
          <li>Contract executes <code className="bg-dark-200 px-1 rounded text-xs">USDC.transfer(contract → merchant)</code></li>
          <li>Stellar ledger confirms in &lt; 5 seconds</li>
          <li>Agent retries original request with <code className="bg-dark-200 px-1 rounded text-xs">X-Payment-Proof</code> header</li>
          <li>Merchant verifies payment and returns <strong className="text-text-primary">HTTP 200</strong></li>
        </ol>
      </section>
