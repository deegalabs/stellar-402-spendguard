  ...
  return (
    <div className="flex flex-col gap-8 max-w-3xl">
      <div className="flex items-center gap-2 text-xs font-mono uppercase tracking-widest text-text-disabled">
        ...
        <span className="text-accent-fg font-bold">Configuration</span>
      </div>

      <div>
        <h1 className="text-4xl font-black text-text-primary tracking-tighter mb-4">Configuration</h1>
      ...
      <section>
        <h2 className="text-xl font-bold text-text-primary mb-4">Backend (.env)</h2>
        <pre className="bg-dark-50 p-4 rounded-xl border border-surface-border font-mono text-sm text-text-primary"><code>{`cp backend/.env.example backend/.env`}</code></pre>

        <div className="overflow-x-auto rounded-xl border border-surface-border mt-4">
              ...
              {envVars.map((v) => (
                <tr key={v.name}>
                  <td className="px-4 py-2">
                    <code className={`text-xs font-bold ${v.secret ? "text-error-fg" : "text-text-primary"}`}>{v.name}</code>
                  </td>
                  <td className="px-4 py-2"><code className="text-xs text-text-secondary">{v.value}</code></td>
      ...
      <section>
        <h2 className="text-xl font-bold text-text-primary mb-4">Frontend (.env.local)</h2>
        <pre className="bg-dark-50 p-6 rounded-xl border border-surface-border font-mono text-sm text-text-primary overflow-x-auto"><code>{`# frontend/.env.local
...
NEXT_PUBLIC_CONTRACT_ADDRESS=CCABMNFY3VKK7BI3YBWXJEE2EXX2NW5S573NASTCFXA6KBXR5PDWFD6E`}</code></pre>
      ...
      <section>
        <h2 className="text-xl font-bold text-text-primary mb-4">Stripe (Test Mode)</h2>
        <p className="text-text-secondary mb-4">
          Stripe integration is simulated in Test Mode. You need Stripe test keys to enable the Liquidity screen:
        </p>
        <pre className="bg-dark-50 p-6 rounded-xl border border-surface-border font-mono text-sm text-text-primary overflow-x-auto"><code>{`STRIPE_PUBLISHABLE_KEY=pk_test_...
...
STRIPE_WEBHOOK_SECRET=whsec_...`}</code></pre>
      ...
      <section>
        <h2 className="text-xl font-bold text-text-primary mb-4">Verify Configuration</h2>
        <p className="text-text-secondary mb-4">After setting up your <code className="bg-dark-200 px-1 rounded text-xs">.env</code>, verify with:</p>
...
# API Docs: http://localhost:3001/api/docs`}</code></pre>
        <p className="text-text-secondary mt-3">Then check the status endpoint:</p>
        <pre className="bg-dark-50 p-4 rounded-xl border border-surface-border font-mono text-sm text-text-primary"><code>{`curl http://localhost:3001/api/status | jq`}</code></pre>
      </section>
    </div>
  );
}
